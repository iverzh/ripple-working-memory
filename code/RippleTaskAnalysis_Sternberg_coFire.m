...
addpath('/space/seh10/6/halgdev/projects/iverzh/ripples/code/bmovie-release-NWB-BIDS/code')
%%
        ...
        %                 
                    ...
                    for uA = 1:length(uRegionA)
                        for uB = 1:length(uRegionB)
                            ...
                            datArip =  ripTrialFullChan(LFPchanNum == uChan(uRegionA(uA)),:);
                            datBrip =  ripTrialFullChan(LFPchanNum == uChan(uRegionB(uB)),:);
                            
                            if isempty(datArip) || isempty(datBrip); continue; end
                            
                            datABcorip = datArip > 0 & datBrip > 0;
                            datABnorip = datArip == 0 & datBrip == 0;
                            
                            datA =  unitTrialFullChan(uRegionA(uA),:); datB =  unitTrialFullChan(uRegionB(uB),:);
                            datA = sum(datA,1) > 0;
                            datB = sum(datB,1) > 0;

                            datA1 = movsum(datA, [coFireWindow coFireWindow]);
                            datB1 = movsum(datB, [coFireWindow coFireWindow]);
                            datAB = double((datA1 > 0) & (datB1 > 0));
    ...
    for iRb = iRa+1:length(regionsPlot)
        ...
        for iiA = iRapl
            for iiB = iRbpl
                ...
                coRimgT         = whole_trials_cross_region_dur{iiA, iiB,2}(:,3);
                noRimgT         = whole_trials_cross_region_dur{iiA, iiB,3}(:,3);
                imgT            = whole_trials_cross_region_dur{iiA, iiB,1}(:,3);
                coRrespT        = whole_trials_cross_region_dur{iiA, iiB,2}(:,4);
                noRrespT        = whole_trials_cross_region_dur{iiA, iiB,3}(:,4);
                respT           = whole_trials_cross_region_dur{iiA, iiB,1}(:,4);
                coRmaintenanceT = whole_trials_cross_region_dur{iiA, iiB,2}(:,2);
                noRmaintenanceT = whole_trials_cross_region_dur{iiA, iiB,3}(:,2);
                maintenanceT    = whole_trials_cross_region_dur{iiA, iiB,1}(:,2);
                baselineT       = whole_trials_cross_region_dur{iiA, iiB,1}(:,1);
                ...
                baselineDur = [baselineDur baselineT'];

                coRimgDur = [coRimgDur coRimgT'];
                noRimgDur = [noRimgDur noRimgT'];
                imgDur = [imgDur imgT'];
                
                coRmaintenanceDur = [coRmaintenanceDur coRmaintenanceT'];
                noRmaintenanceDur = [noRmaintenanceDur noRmaintenanceT'];
                maintenanceDur = [maintenanceDur maintenanceT'];
                
                coRrespDur = [coRrespDur coRrespT'];
                noRrespDur = [noRrespDur noRrespT'];
                respDur = [respDur respT'];
...
%         coRresp(isinf(coRresp)) = nan;
%         noRresp(isinf(noRresp)) = nan;
%         resp(isinf(resp)) = nan;
%         coRimg(isinf(coRimg)) = nan;
%         noRimg(isinf(noRimg)) = nan;
%         img(isinf(img)) = nan;
%         coRmaintenance(isinf(coRmaintenance)) = nan;
%         noRmaintenance(isinf(noRmaintenance)) = nan;
%         maintenance(isinf(maintenance)) = nan;
%         baseline(isinf(baseline)) = nan;
...
%                    arrayfun(@(X) mean(resp(uPairsID == X), 'omitnan'), unique(uPairsID))'] ;
        ...
        tRespID = [arrayfun(@(X) sum(baseline(uPairsID == X), 'omitnan') / sum(baselineDur(uPairsID == X), 'omitnan'), unique(uPairsID))', ...
                   arrayfun(@(X) sum(coRimg(uPairsID == X), 'omitnan') / sum(coRimgDur(uPairsID == X), 'omitnan'), unique(uPairsID))', ...
                   arrayfun(@(X) sum(noRimg(uPairsID == X), 'omitnan') / sum(noRimgDur(uPairsID == X), 'omitnan'), unique(uPairsID))', ...
                   arrayfun(@(X) sum(img(uPairsID == X), 'omitnan') / sum(imgDur(uPairsID == X), 'omitnan'), unique(uPairsID))', ...
                   arrayfun(@(X) sum(coRmaintenance(uPairsID == X), 'omitnan') / sum(coRmaintenanceDur(uPairsID == X), 'omitnan'), unique(uPairsID))', ...
                   arrayfun(@(X) sum(noRmaintenance(uPairsID == X), 'omitnan') / sum(noRmaintenanceDur(uPairsID == X), 'omitnan'), unique(uPairsID))', ...
                   arrayfun(@(X) sum(maintenance(uPairsID == X), 'omitnan') / sum(maintenanceDur(uPairsID == X), 'omitnan'), unique(uPairsID))', ...
                   arrayfun(@(X) sum(coRresp(uPairsID == X), 'omitnan') / sum(coRrespDur(uPairsID == X), 'omitnan'), unique(uPairsID))', ...
                   arrayfun(@(X) sum(noRresp(uPairsID == X), 'omitnan') / sum(noRrespDur(uPairsID == X), 'omitnan'), unique(uPairsID))', ...
                   arrayfun(@(X) sum(resp(uPairsID == X), 'omitnan') / sum(respDur(uPairsID == X), 'omitnan'), unique(uPairsID))'] ;
