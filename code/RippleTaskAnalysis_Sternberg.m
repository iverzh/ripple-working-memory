...
%     chan_labels = strsplit(micrObj.chan_locations, micrObj.chan_locations(38))
    ...
    modifier = '1kHz_template_baselineNorm_z30';
    modifier = '1kHz_template_z25';
%     modifier = 'LG_1kHz_z25';
%     modifier = 'HG_1kHz_z25';
...
%     rippAll = mean(zscore(rippMask,0,2));
...
%         dat = ( rippAll - mean(rippAll, 'omitnan') ) / std(rippAll, 'omitnan');
        dat = rippAll(respTime+winResp(1):respTime+winResp(2));
        recog_trials_resp_all(cTrial, :) = (dat); %/mean(rippAll(baselineMask), 'omitnan');
...
%         recog_trials_resp_all(cTrial, :) = dat;
        ...
        whole_trial_all(cTrial,:) = trialFull; % / mean(rippAll(baselineMask), 'omitnan');
%         whole_trial_units_all(cTrial,:) = smoothdata(sum(unitTrialFullChan, 1, 'omitnan'), 'gaussian', 100) / size(unitTrialFullChan,1);
...
%             whole_trials_BP_region_all(:,:,iR, 1)
...
%                 probeRipples = sum(rippMask(chRegion(ii),probeTime:probeTime+1250));
%                 probe_trials_all{trialLoad,iR} = [probe_trials_all{trialLoad,iR}; [countChPerRegion(iR)+ii probeRipples]];
%             end
...
%     dat1 = probe_trials_units_all{1,iR};
%     dat3 = probe_trials_units_all{3,iR};
%     
    dat1 = probe_trials_all{1,iR};
    dat3 = probe_trials_all{3,iR};
   
    chRegion = unique(dat1(:,1));
    P1 = []; P3 = []; P = [];
    for ii = 1:length(chRegion)
        ch = chRegion(ii);
%         [H,P(ii),CI] = ttest2(dat1(dat1(:,1) == ch,2),dat3(dat3(:,1) == ch,2));
        P1(ii) = mean(dat1(dat1(:,1) == ch,4), 'omitnan');
        P3(ii) = mean(dat3(dat3(:,1) == ch,4), 'omitnan');
        
        [P(ii),H] = ranksum(dat1(dat1(:,1) == ch,4),dat3(dat3(:,1) == ch,4));
        [H,P(ii),CI] = ttest2(dat1(dat1(:,1) == ch,4),dat3(dat3(:,1) == ch,4), 'tail', 'left');
%         [stats, df, P(ii), surrog] = statcond({dat1(dat1(:,1) == ch,4)', dat3(dat3(:,1) == ch,4)'},'paired','off','tail', 'one', 'method', 'perm', 'naccu', 10000, 'verbose','off');
    end
    
%     Pbundle1 = nan(1, length(subjs));
%     Pbundle3 = nan(1, length(subjs));
%     dat = whole_trials_region_all(:,:,iR);
%     subjs = unique(subjID);
%     for ii = 1:length(subjs)
%         s = subjs(ii);
%         dat1 = mean(dat(load_trials_all == 1 & subjID == s,taskMarkers(end)+1:taskMarkers(end)+1200),2);
%         dat3 = mean(dat(load_trials_all == 3 & subjID == s,taskMarkers(end)+1:taskMarkers(end)+1200),2);
%         Pbundle1(ii) = mean(dat1);
%         Pbundle3(ii) = mean(dat3);
%         if all(isnan(dat1)); continue; end
%         [Pbundle(ii),H] = ranksum(dat1,dat3);
%         plot(1,mean(dat1), 'o'); hold on;
%         plot(3,mean(dat3), 'o'); hold on;
%         plot([1 3],[mean(dat1) mean(dat3)], '-'); hold on;
%     end
%     
%     sum(P3 > P1) / sum(~isnan(P1))
%     sum(P < 0.05) / length(P)
     fprintf('%i / %i \n', sum(P3 > P1) , length(P3))
    subplot(1,5,iR)
    
    histogram([P3-P1]/mean(P1), 50);
    vline(0)
...
%                 coR(iT, iR, 1) = max(whole_trial_all(iT, 1:taskMarkers(1)));
...
%                 coR(iT, iR, 6) = mean(whole_trial_all(iT, taskMarkers(5):taskMarkers(5)+1e3), 'omitnan');
                coR(iT, iR, 6) = mean(whole_trial_all(iT, taskMarkers(5):taskMarkers(5)+1e3), 'omitnan');
%                 coR(iT, iR, 6) = max(whole_trial_all(iT, taskMarkers(5):taskMarkers(5)+1e3));
...
%                 coR(iT, iR, 6) = max(whole_trial_all(iT, taskMarkers(5):taskMarkers(5)+1e3));
    ...
    for iTm = [1,2,5,6]
%         [pLoadCoR(iR, iTm),STATS] = ranksum(coR(plot_condition, iR, iTm), coR(~plot_condition, iR, iTm), 'tail', 'both');
%         [pLoadFR(iR, iTm) ,STATS] = ranksum(FR(plot_condition, iR, iTm),   FR(~plot_condition, iR, iTm), 'tail', 'both');
...
%         [pLoadCoR(iR, iTm)] = ranksum(dat(plot_condition), dat(~plot_condition));
%         [~, pLoadCoR(iR, iTm)] = ttest2(dat(plot_condition), dat(~plot_condition));
%         [~,~,pLoadCoR(iR, iTm)] = statcond({dat(plot_condition & ~isnan(dat))',dat(~plot_condition & ~isnan(dat))'},'paired','off','method', 'perm', 'naccu', 10000, 'verbose','off');
...
%         [~,pLoadFR(iR, iTm)] = ttest2(dat(plot_condition), dat(~plot_condition));
%         [pLoadFR(iR, iTm)] = ranksum(dat(plot_condition), dat(~plot_condition));
%         [~,~,pLoadFR(iR, iTm)] = statcond({dat(plot_condition & ~isnan(dat))',dat(~plot_condition & ~isnan(dat))'},'paired','off','method', 'perm', 'naccu', 10000, 'verbose','off');
...
%         lme = fitlme(tab, 'rates ~ rates_baseline + (1|subject)');   
...
% iR = 3;
xx = squeeze(FR(:, iR, iT)); 
yy = squeeze(coR(:, iR, iT));
keep = ~isnan(xx) & ~isnan(yy);
xx = xx(keep); yy = yy(keep);
[RHO, P] = corr(xx(:), yy(:));

effect = nan(5,max(subjID));
%     base = squeeze(coR(:, iR, 1))';
%     d = mean(squeeze(coR(:, iR, iT)),2, 'omitnan')';
%     eff = cohens_d(d(plot_condition), d(~plot_condition))% pct = [d-base]./base;
%     eff = cohens_d(d, base)% pct = [d-base]./base;



base = [];
for iS = 1:max(subjID)
%         if all(isnan(d(subjID == iS))); continue; end
    baseS = squeeze(coR(subjID == iS, iR, 1))';
    base = [base repmat(mean(baseS), [1 length(baseS)])];
%     d = squeeze(coR(:, iR, iT))';
%     [mean(d, 'omitnan') - mean(base, 'omitnan')] / mean(base, 'omitnan')
%     [mean(d(plot_condition), 'omitnan') - mean(d(~plot_condition), 'omitnan')] / mean(d, 'omitnan')
%     [H,P] = ttest2(d(plot_condition), d(~plot_condition))    % pct = [d-base]./base;
%         effect(iR,iS) = cohens_d(d(plot_condition & subjID == iS), d(~plot_condition & subjID == iS));    % pct = [d-base]./base;
%      [d2, stats2] = cohens_d_robust(d(plot_condition), d(~plot_condition), 'robust', true, 'bootstrap', true)
% mean(pct(isfinite(pct)), 'omitnan')
end
baseSem = std(coR(subjID == iS, iR, 1)) / sqrt(length(base));
dResp = [mean(squeeze(coR(:, iR, 6)),2, 'omitnan')'-base]';
dMaint = [mean(squeeze(coR(:, iR, 5)),2, 'omitnan')'-base]';

[mean(dResp(plot_condition), 'omitnan') - mean(dResp(~plot_condition), 'omitnan')] / baseSem
[mean(dMaint(plot_condition), 'omitnan') - mean(dMaint(~plot_condition), 'omitnan')] / baseSem


% [mean(d(plot_condition), 'omitnan') - mean(d(~plot_condition), 'omitnan')] / mean(d(~plot_condition), 'omitnan')
% %     eff = cohens_d(d(plot_condition), d(~plot_condition))
% [g, cohens_d, pooled_sd] = hedges_g(d(plot_condition), d(~plot_condition))
% [h,p,ci,stats] = ttest2(d(plot_condition), d(~plot_condition))

T = table(subjID(~isnan(dResp)), plot_condition(~isnan(dResp)), dResp(~isnan(dResp)),dMaint(~isnan(dResp)), ...
                 'VariableNames', {'subject',               'condition',                     'rates_resp',            'rates_maintenance'});
% [effect_sizes] = lme_effect_size(T,    'rates_resp', 'condition','subject');
% [effect_sizes] = lme_effect_size(T,    'rates_maintenance', 'condition','subject');
 


[X, Y, T, AUC] = perfcurve(plot_condition, dResp, 1);
    
% Plot using built-in function results
figure('Name', 'ROC Curve - Built-in Function');
plot(X, Y, 'b-', 'LineWidth', 2);
hold on;
plot([0 1], [0 1], 'r--', 'LineWidth', 1); % Diagonal reference line
...
taskMarkers = [500 2600 4700 6800 9400]+500;
if true %computeTF
    erspDiff = nan(200,400,max(subjID), length(regions));
    erspLoad = nan(200,400,max(subjID), length(regions));
    for iR = 1:length(regions)

        regionERP = whole_trials_ERP_region_all(:,:,iR);
        regionERP(cTrial:end,:) = [];
        for iSub = 1:max(subjID)
            regionERPsubj = regionERP(subjID == iSub,(taskMarkers(end)-1e3):end);
            loadSubj = load_trials_all(subjID == iSub);
            loadSubj(isnan(regionERPsubj(:,1)),:) = [];
            regionERPsubj(isnan(regionERPsubj(:,1)),:) = [];

            if isempty(regionERPsubj); continue; end

            [xx yy] =  find(sum(isnan(regionERPsubj),2) > 5e2);
            regionERPsubj(xx,:) = [];
            loadSubj(xx,:) = [];
            [ersp1, ~, ~, times, freqs, ~] = newtimef(regionERPsubj(loadSubj==1,:)', size(regionERPsubj,2), [-1e3   size(regionERPsubj,2)-1e3], fs,...
                                        'cycles', [0], ...
                                        'alpha', 0.05, ...
                                        'plotitc', 'off',...
                                        'winsize', 100, ...
                                        'timesout', floor(size(regionERPsubj,2)/10),...
                                        'maxfreq', fs/2, 'freqs', [1 fs/2], 'freqscale', 'log',...
                                        'plotphase', 'off', ...
                                        'padratio', 4, 'naccu', 100,'baseline', [-1e3 -50], 'baseboot', [-1e3 -50], 'mcorrect', 'fdr');
            [ersp3, ~, ~, times, freqs, ~] = newtimef(regionERPsubj(loadSubj==3,:)', size(regionERPsubj,2), [-1e3   size(regionERPsubj,2)-1e3], fs,...
                                        'cycles', [0], ...
                                        'alpha', 0.05, ...
                                        'plotitc', 'off',...
                                        'winsize', 100, ...
            ...
            erspDiff(:,:,iSub,iR) = ersp3 - ersp1;
            erspLoad(:,:,iSub,iR) = ersp3 ;
...
%%
iR = 5;
hand = figure('Position', [316 30 815 564]);
% ersp = mean(ERSPcond1 - ERSPcond2, 3);
erspMu = mean(erspDiff(:,:,:,iR),3, 'omitnan');
im = imagesc([-1e3   size(regionERPsubj,2)-1e3], [1 fs/2],erspMu);hold on
ax = gca;
ax.YDir = 'normal';
ax=imgca;
set(ax,'YScale', 'log')
...
v  = vline(0, 'k:');
v.LineWidth = 1.5;
xlabel('time from aud. stim.(ms)')
ylabel('frequency (Hz)')
c=colorbar;
c.Label.String = 'ERSP (\Delta dB)';
...
%                               std(plot_data, 'omitnan')/sqrt(sum(~plot_condition)), 'b', 'nan', 'gap'); hold on;
% bf.FaceAlpha = 0.7;
% vline(0)
...
%     std(recog_trials_units_all(plot_condition,:), 'omitnan')/sqrt(sum(plot_condition)), 'r', 'nan', 'gap'); hold on;
% bf.FaceAlpha = 0.7;
% [bl, bf] = boundedline(win(1):win(2), mean(recog_trials_units_all(~plot_condition,:), 'omitnan'), ...
%     std(recog_trials_units_all(~plot_condition,:), 'omitnan')/sqrt(sum(~plot_condition)), 'b', 'nan', 'gap'); hold on;
% bf.FaceAlpha = 0.7;
% vline(0)
% ylim([-0.3 0.3])
% xlim(Lim)
