...
addOptional(p, 'mediansize', 3)
addOptional(p, 'meansize', 5)
...
gpos = gpos + confs.xshift;
cpos = cpos + confs.xshift; 
h.gpos = gpos;
h.cpos = cpos; 
...
for g = 1:num_groups 
    ...
    for k = 1:num_locs
        ...
        elseif confs.fill==1
            ...
            set(h.bx(k,g),'FaceAlpha',confs.boxalpha, 'EdgeColor', confs.colors(g,:)); 
            hold on;

            % draw the median
            h.md(k,g) = line(Xx(1,:), box_mdcor(wk),...
                'color',confs.colors(g,:), 'LineWidth', 2);
             plot(mean(Xx(1,:)),mean(box_mdcor(wk)),'ks',...
                    'MarkerFaceColor',confs.colors(g,:),'Color', confs.colors(g,:), 'LineWidth', 1.5,  'MarkerSize', confs.mediansize); hold on;
            % draw the mean
            if confs.mean==1
%                 h.mn(k,g) = line(Xx(1,:),box_mncor(wk),'LineStyle',':',...
%                     'color','k','LineWidth', 1.5);
                
                h.mn(k,g) = plot(mean(Xx(1,:)),mean(box_mncor(wk)),'ko',...
                    'MarkerFaceColor','w','Color', confs.colors(g,:),'LineWidth', 1.5, 'MarkerSize', confs.meansize);
            end
        end        
        ...
        if confs.whiskers==1            
            h.wh(k,g,:) = plot(whi_xcor(:,k),whi_ycor(:,1,k),'k-',... 
                ...
                'LineWidth',1, 'Color', confs.colors(g,:));                              
        end 
